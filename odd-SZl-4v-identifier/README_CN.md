...
- `szl_odd_solver.py`：奇数模 $l$ 的 $SZ_l$ 判定与 $\beta$-定向求解器。
- `generate_nonisomorphic_4v_szl.py`：对**给定的奇数 $l$**，枚举 4 顶点、$3(l-1)$ 条边（计重）的无向多重图（约束：每点最小度 $\ge l-1$，每对顶点边重数 $\le l-2$，连通）；按同构去重后绘制总览图；对每个图调用求解器进行 $SZ_l$ 检测，并对非 $SZ_l$ 的图输出**所有**不可行的 $\beta$。
...
`generate_nonisomorphic_4v_szl.py` 会动态导入同目录下的 `szl_odd_solver.py` 来获取 `SZlOddSolver` 类，因此两者需要处于同一目录（或自行修改导入路径）。

目录中的 `nonisomorphic_4v12e_l5_overview.png`（或对其它 $l$ 的 `nonisomorphic_4v*e_l*_overview.png`）为运行 `generate_nonisomorphic_4v_szl.py` 时生成的总览图。读者可通过指定 `--modulus l` 自行运行脚本以生成对应 $l$ 的图像。
...
默认 $l=5$。例如指定 $l=7$：`python generate_nonisomorphic_4v_szl.py --modulus 7`。

运行后将：

- 弹出 Matplotlib 总览图窗口（所有非同构代表元的网格图）
- 在“运行脚本时的当前工作目录”保存 PNG 总览图：
  - `nonisomorphic_4v{3(l-1)}e_l{l}_overview.png`（例如 $l=5$ 时为 `nonisomorphic_4v12e_l5_overview.png`）
- 在控制台打印 $SZ_l$ 判定结果；若某图不是 $SZ_l$，会打印该图的**所有**不可行 $\beta$（向量形式）。
- 将上述运行输出同时写入当前工作目录下的结果文件 `nonisomorphic_4v{3(l-1)}e_l{l}_results.txt`，便于日后查阅。使用 `--no-log` 可关闭写入结果文件；使用 `--no-show` 可仅保存图像不弹窗。
...
### 5.6 `get_all_infeasible_betas(verbose=False, max_beta=None)`：收集所有不可行 $\beta$

枚举所有合法 $\beta$，返回**所有**使得 $\beta$-定向不存在的 $\beta$ 的列表（用于对非 $SZ_l$ 图输出全部反例）。

---

## 6. `generate_nonisomorphic_4v_szl.py`：总体功能

该脚本把“构造实验数据集 + 可视化 + 批量判定”合并在一起。对**给定的奇数 $l$**（默认 5，可通过 `--modulus` 指定），主要任务是：

1. 枚举满足约束的 4 顶点、$3(l-1)$ 条边（计重）多重图：每点最小度 $\ge l-1$，每对顶点边重数 $\le l-2$，连通
2. 在顶点置换意义下去掉同构重复（保留代表元）
3. 绘制所有代表元的总览图，并保存 PNG
4. 对每个代表元调用 `SZlOddSolver(..., l)` 判定是否为 $SZ_l$；若非 $SZ_l$，打印该图的**所有**不可行 $\beta$

---

## 7. `generate_nonisomorphic_4v_szl.py`：实现流程（按模块）

### 7.1 枚举空间定义（精确）

对给定的奇数 $l$，顶点固定为 $`\{1,2,3,4\}`$，只考虑 6 个无序点对：

$$
(1,2),(1,3),(1,4),(2,3),(2,4),(3,4)
$$

对每对分配重数 $`k_{uv}\in\{0,1,\dots,l-2\}`$（每对最多 $l-2$ 条边），并筛选：
...
### 7.4 保存总览图（PNG）与结果文件（TXT）

脚本会保存总览图到当前工作目录（即你运行 `python ...` 时所在目录）：

- `nonisomorphic_4v{3(l-1)}e_l{l}_overview.png`（例如 $l=5$ 时为 `nonisomorphic_4v12e_l5_overview.png`）

并将与控制台相同的运行输出写入同目录下的结果文件：

- `nonisomorphic_4v{3(l-1)}e_l{l}_results.txt`（例如 $l=5$ 时为 `nonisomorphic_4v12e_l5_results.txt`）

使用 `--no-log` 可关闭结果文件写入。控制台会打印总览图与结果文件的实际保存路径。
...
- 构造 `nx.MultiGraph`
- `solver = SZlOddSolver(Gm, l)`
- `infeasible_betas = solver.get_all_infeasible_betas(verbose=False)`

若存在不可行 $\beta$（即图不是 $SZ_l$），则打印：

- 图编号
- 6 个点对的边重数（总和 $3(l-1)$）
- 顶点度
- **所有**不可行 $\beta$ 的向量形式（按 `solver.vertices` 顺序）及总数
...
- **确定性**：
  ...
  - `generate_nonisomorphic_4v_szl.py`：同构去重采用全置换 canonical key，完全确定
