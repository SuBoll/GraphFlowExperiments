...
### 4.2 Sign convention and constant terms $`C_v`$, $`S(v)`$

For each edge bundle $`e=(u,v)`$ (normalized with $`u<v`$), define the sign:
- $`\mathrm{sign}(u,e)=+1`$ ($`u`$ is the smaller endpoint)
